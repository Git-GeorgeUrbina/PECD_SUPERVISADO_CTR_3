"""
🏦 Evaluación de Incumplimiento de Pagos App - Demo de ML en Producción
================================================
...
# ============================================
st.set_page_config(
    page_title="Evaluación de Incumplimiento de Pagos - PECD UNI",
    page_icon="💰",
    layout="wide",
...
FEATURE_CONFIG = {
    ...
    "person_income": {
        "label": "💷 Ingreso anual (S/)",
        "description": "Ingresos anuales (en soles)",
        "min": 1.00,
        "max": 8000000.00,
        "default": 1000.00,
        "step": 100.00,
    ...
    "person_home_ownership": {
        ...
        "description": "Tipo de propiedad de la vivienda",        
        "default": ""
    },
    "person_emp_length": {
        "label": "📆 Antigüedad laboral (años)",
        "description": "Antigüedad laboral (en años)",
        "min": 1,
        "max": 50,
        "default": 1,
        "step": 1,
    ...
    "loan_intent": {
        ...
        "description": "Intención/motivo de préstamo",
        "default": ""
    },
    "loan_grade": {
        "label": "✏️ Grado del préstamo",
        "description": "Calificación/grado del préstamo (A a G en orden de riesgo)",
        "default": ""
    },
    "loan_amnt": {
        "label": "🪙 Monto de préstamo (S/)",
    ...
    "loan_percent_income": {
        "label": "🔗 Porcentaje del ingreso destinado al préstamo (%)",
        "description": "Porcentaje de ingresos del préstamo (Relación entre el monto del préstamo y el ingreso del solicitante)",
        "min": 0.00,
        "max": 1.00,
        "default": 0.05,
        "step": 0.01,
    ...
    "cb_person_default_on_file": {
        "label": "🔎 Historial de morosidad (si/no)",
        "description": "Historial de morosidad. ¿ha tenido incumplimientos anteriores en su historial de crédito? (sí/no).",
        "default": ""
    },
    "cb_person_cred_hist_length": {
        "label": "⌛ Historial crediticio (años)",
        "description": "Antigüedad del historial crediticio",
        "min": 1,
        "max": 30,
        "default": 1,
        "step": 1,
...
def create_gauge_chart(probability: float) -> go.Figure:
    """Crea un gráfico de velocímetro para la probabilidad de incumplimiento de pago."""
...
def create_feature_impact_chart(input_values: dict, feature_config: dict) -> go.Figure:
    ...
    FEATURES_A_EXCLUIR = ['person_home_ownership', 'loan_intent', 'loan_grade','cb_person_default_on_file']

    for feat_name, config in feature_config.items():
        # *** ESTE ES EL CÓDIGO CLAVE PARA EXCLUIR ***
        if feat_name in FEATURES_A_EXCLUIR:
            continue  # Esto salta inmediatamente al siguiente elemento del bucle
        # **********************************************
...
def main():
    ...
    <div class="main-header">
        <h1>💰 Sistema de Evaluación de Incumplimento de Pagos</h1>
        <h4>Control 3: Modelo XGBoost</h4>
    ...
    with col_input:
        ...
        for feat_name, config in FEATURE_CONFIG.items():
            ...
            if isinstance(config["default"], int):            
                ...
                if feat_name == "person_home_ownership":                    
                    input_values[feat_name] = st.selectbox('Tipo de vivienda', ['MORTGAGE','OTHER','OWN','RENT'])
                elif feat_name == "loan_intent":                    
                    input_values[feat_name] = st.selectbox('Motivo de préstamo', ['DEBTCONSOLIDATION','EDUCATION','HOMEIMPROVEMENT','MEDICAL','PERSONAL','VENTURE'])
                elif feat_name == "loan_grade":                    
                    input_values[feat_name] = st.selectbox('Grado del préstamo', ['A','B','C','D','E','F','G'])
                elif feat_name == "cb_person_default_on_file":
                     input_values[feat_name] = st.selectbox('Historial de morosidad (si/no)', ['N','Y'])
    ...
    with col_result:
        if predict_button:
            ...
            st.markdown("### 📊 Resultado del Análisis")
            st.info(
                "👈 Complete los datos del cliente y presione **EVALUAR** para obtener el análisis."
            )
            ...
            <div style="text-align: center; padding: 50px; color: #6c757d;">
                <h1>⏳</h1>
                <p>Esperando datos del cliente...</p>
            </div>
            """,
                unsafe_allow_html=True,
            )
